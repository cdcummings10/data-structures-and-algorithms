...
const countNumberOfElements = (arr) => arr.reduce((acc, value, index) => acc = index + 1);
...
const reversedString = (str) => {
  // Solution code here...
  let answer = str.split('').reduce((acc, value) =>{
    acc.unshift(value);
    return acc;
  }, [])
  return answer.join('')
...
const countNumberOfChildren = (arr) => {
  ...
  return arr.reduce((acc, value) => {
    if (Array.isArray(value.children)) {acc += value.children.length};
    return acc;
  }, 0)
};
