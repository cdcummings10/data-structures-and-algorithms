...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < str.length+1; i++){
    result.unshift(str.slice((str.length - i)));
  }
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(item => {
    let recipeArr = item.split(' ');
    let add = '';
    for (let i = 2; i < recipeArr.length; i++){
      if (i === 2){
        add += `${recipeArr[i]}`
      }
      else {
...
const stepActions = (recipe) => {
  // Solution code here...
  return recipe.steps.map(x => x.split(' ')[0]);
};
...
const removeEvenValues = (arr) => {
  ...
  for (let i = 0; i < length; i++){
    if (parseInt(arr[i - current]) % 2 === 0){
      arr.splice(i - current, 1);
      current++;
    }
  }
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (str.length < numberOfCharacters){
    ...
    return str.slice(0, (0 -numberOfCharacters));
  }
};
