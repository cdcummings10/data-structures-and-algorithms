...
const toTitleCase = (arr) => {
  ...
  let answer = [];
  arr.forEach(element => {
    answer.push(element.replace(element.charAt(0), element.charAt(0).toUpperCase()));
  });
  return answer;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let woop = arr.filter(value => parseInt(value.mass) > parseInt(arr[0].mass));
  return woop.map(value => value.name).join(' - ');
};
