...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(num => num.slice(1,4) + num.slice(6,9) + num.slice(10))
};
...
const allHappy = (arr) => {
  // Solution code here...
  let woop = arr.filter(string => string.includes(':)'))
  return (woop.length === arr.length) ? true : false;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let answer = false;
  let woop = arr.filter(element => element.includes(target))
  if (woop.length === arr.length){
    answer = true;
  }
  return answer;
};
...
const unenrollBrook = (arr) => {
  ...
  return arr.map(value => {
    return value.filter(woop => !woop.includes('Brook'))
  })
};
